class User{
    constructor(firstName, lastName, DoB, departing, arriving, leaveDate, returnDate, bags, meal, extra, dayDiff, cost, age){
...
function addToList(){
    ...
    //age
    let birthMonth = DoB.split('-')[1];
    let birthDay = DoB.split('-')[2];
    let today = new Date();
...
function search(){
    ...
    for(i = 0; i < userList.length; i++){
        if(searchUser[0] == userList[i].firstName && searchUser[1] == userList[i].lastName){
            ...
            document.getElementById("outextras").value = userList[i].extra;
